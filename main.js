 ...
 */
    ...
    function createPaint(parent) {
        ...
        let panel = elt("div", {class: "pictruepanel"}, canvas);
    ...
    controls.brushSize = function (cx) {
        ...
        sizes.forEach(function (sizes) {
            select.appendChild(elt("option", {value: sizes}, sizes + "pixels"));
        });
    ...
    controls.openURL = function () {
        let input = elt("input", {type: "text"});
        let form = elt("from", null, "Open URL: ", input, elt("button", {type: "submit"}, "load"));
    ...
    tools.Text = function (event, cx) {
        ...
        if (text) {
            let pos = relativePos(event, cx.canvas);
            cx.font = Math.max(7, cx.lineWidth) + "px sans-self";
